  ...
  function (module, exports, __webpack_require__) {
    ...
    var render = function render(tagType, res) {
      ...
      for (var j = 0, len2 = res.list.length; j < len2; j++) {
        ...
        for (var i = 0, len = data.link.length; i < len; i++) {
          if (tagType === "1") {
            https://gitee.com/lmdream/photo_album
            var minSrc = "https://gitee.com/lmdream/photo_album/raw/master/min_movie/" + data.link[i];
            var src = "https://gitee.com/lmdream/photo_album/raw/master/movie/" + data.link[i];
          } else if (tagType === "2") {
            var minSrc = "https://gitee.com/lmdream/photo_album/raw/master/min_lyric/" + data.link[i];
            var src = "https://gitee.com/lmdream/photo_album/raw/master/lyric/" + data.link[i];
          } else {
            var minSrc = "https://gitee.com/lmdream/photo_album/raw/master/min_photos/" + data.link[i];
            var src = "https://gitee.com/lmdream/photo_album/raw/master/photos/" + data.link[i];
          }
          ...
          liTmpl += '<figure class="thumb" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">\
                  <a href="' + src + '" itemprop="contentUrl" data-size="640x640" data-type="' + type + '" data-target="' + src + '">\
                    <img class="reward-img" data-type="' + type + '" data-size="640x640" data-src="' + minSrc + '" src="' + src + '" itemprop="thumbnail" onload="lzld(this)">\
                  </a>\
    ...
    function loadData(tagType, success) {
      if (!searchData) {
        ...
        if (tagType === "1") {
          xhr.open('GET', './movie.json?t=' + new Date(), true);
        } else if (tagType === "2") {
          xhr.open('GET', './lyric.json?t=' + new Date(), true);
        } else {
    ...
    var initPhotoSwipeFromDOM = function initPhotoSwipeFromDOM(gallerySelector) {
      ...
      var parseThumbnailElements = function parseThumbnailElements(el) {
        ...
        for (var i = 0; i < numNodes; i++) {
          ...
          var imga = new Image();
          imga.src = linkEl.getAttribute('href');
            // create slide object
            item = {
              src: linkEl.getAttribute('href'),
              w: parseInt(imga.width, 10), //直接在这里我们计算图片的宽高即可。
              h: parseInt(imga.height, 10)
            };
